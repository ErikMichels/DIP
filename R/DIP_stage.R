...
DIP_stage <- function(new_data) {
  ...
  rows_with_more_than_one_missing <- new_data$ID[rowSums(is.na(new_data[expected_vars])) > 0]
  if (length(rows_with_more_than_one_missing) > 0) {
  ...
  scatter_3d <- plotly::plot_ly(data = results_df, x = ~DIP1_Prob * 100, y = ~DIP2_Prob * 100, z = ~DIP3_Prob * 100, type = 'scatter3d', mode = 'markers',
                                color = ~DIP,
                                colors = c("#8BCCF1", "#896DB0", "#006E78"),
                                size = 2,
                                sizes = 100,
                                alpha = 1) %>%
    plotly::layout(scene = list(xaxis = list(title = "DIP1 Prob (%)"),
