...
cDIP_impute <- function(new_data) {
  ...
  p <- ggplot(results_df, aes(y = prediction, color = prediction, alpha = 0.9)) +
    geom_beeswarm(aes(x = factor(1)), cex = 3, alpha = 0.9) +
    scale_color_gradient2(
      low = "#8BCCF1", mid = "#896DB0", high = "#006E78",
      midpoint = 0.5831,
      breaks = c(0, 0.25, 0.5, 0.75, 1.0),
      labels = c("0 (Minor)", "0.25", "0.50", "0.75", "1 (Major)"),
      limits = c(0, 1)
    ) +
    theme_minimal() +
    theme(
    ...
    labs(x = NULL, y = "continuous Dysregulated Immune Profile (cDIP)", color = "cDIP") +
    coord_flip()
  print(p)
